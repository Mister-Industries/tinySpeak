...
int16_t wav_buffer[I2S_READ_LEN / 2]; // Mono 16-bit output (half the stereo frames)
...
// ---------------------------------------------------------------
// Pipeline: OpenAI TTS (Text-to-Speech) -> SD -> Play
// ---------------------------------------------------------------

// Streams the TTS MP3 response directly to SD before playing.
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        if (input[0] == 'c') {
            if (isRecording) stopRecording(); // finalize any in-progress recording first
            stopSpeaking();
            runConfigWizard();
            connectWiFi();
            printMenu();
